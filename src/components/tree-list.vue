  ...
  <div class="tree-list" ref="wrapper">
    ...
    <div>
      <div
        ...
        :key="item.oid"
        @click="sendOrg(item.oid, 'join')"
      >
        <div class="card-left">
          <div
            class="select-icon"
            v-show="
              type === 'edit' && userType === 'user' && item.parentOid !== 0
            "
          ...
          >
            <span>(</span>
            <span v-show="userNum[item.oid] ? userNum[item.oid].select : false"
      ...
      <div
        slot="list"
        class="dash"
        v-show="userData.length && orgData.length"
      ></div>
      <!-- userlist -->
      <div
        slot="list"
...
export default {
  name: "treeList",
  props: ["type", "orgData", "userData", "selectData", "userType", "userNum"],
  data() {
    return {
      avatarSelect: {
        backgroundImage:
